...
//    Emitter<String> emitter;
   ...
   // EntityManager em;
    ...
    public UserLogResponse doCreate(UserProfileRequest rqx) 
    {
        ...
        try 
        {
             ...
             UserProfileRequestObj fromJson = new UserProfileRequestObj(rqx);
             ProfileSetup doLookUpById = profilesRepo.doLookUpByName(fromJson.emailAddress, fromJson.mobileNo);
                
                 if( doLookUpById != null)
                 {
                    return new UserLogResponse(false, ErrorCodes.USER_ALREADY_EXIST,ErrorCodes.doErrorDesc(ErrorCodes.USER_ALREADY_EXIST), null, null);
            
                 }
           
                ProfileSetup doLog = profilesRepo.doLog(rqx); //doFindByDescription(jsonStr)
                
                if(doLog != null)
                {
                   // return Response.ok().entity(doLog.toJson()).build();
                    // confs.add(doLog);
                     return new UserLogResponse(true, ErrorCodes.SUCCESSFUL,ErrorCodes.doErrorDesc(ErrorCodes.SUCCESSFUL), doLog, null);
                }
                else
                {
                    ...
                    return new UserLogResponse(false,ErrorCodes.DATABASE_ERROR, ErrorCodes.doErrorDesc(ErrorCodes.DATABASE_ERROR), null,null); 
                ...
                if(e.getCause() instanceof CMFBException)
                {
                    ...
                    CMFBException exp =   (CMFBException) e.getCause();
                     ...
                     return new UserLogResponse(false,ErrorCodes.SYSTEM_ERROR, ErrorCodes.doErrorDesc(ErrorCodes.SYSTEM_ERROR), null,e.getMessage()); 
                ...
                else if((e.getCause() instanceof java.sql.SQLSyntaxErrorException) || ( e.getCause() instanceof  org.hibernate.HibernateException) || ( e.getCause() instanceof  org.hibernate.exception.ConstraintViolationException) || ( e.getCause() instanceof  java.sql.SQLIntegrityConstraintViolationException) || (e.getCause() instanceof java.sql.SQLIntegrityConstraintViolationException)  || (e.getCause() instanceof jakarta.persistence.PersistenceException))
                {
                     //throw new HelloException(ErrorCodes.DATABASE_ERROR, e.getMessage(),prodOrDev);
                      ...
                      return new UserLogResponse(false,ErrorCodes.DATABASE_ERROR, ErrorCodes.doErrorDesc(ErrorCodes.DATABASE_ERROR), null,e.getMessage()); 
               
                }
                else if((e.getCause() instanceof java.io.IOException))
                {
                    return new UserLogResponse(false,ErrorCodes.IO_EXCEPTION, ErrorCodes.doErrorDesc(ErrorCodes.IO_EXCEPTION), null,e.getMessage()); 
    ...
    public PaymentSetupResponse doEdit(PaymentSetupEditRequest jsonStr) {
        try 
        {
                ...
                PaymentSetup doLog = profilesRepo.doSync(fromJson, doLookUpById); //doFindByDescription(jsonStr)
